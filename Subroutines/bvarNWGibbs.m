...
function [result, VAR] = bvarNWGibbs(VARoption,BVARoption,VAR,result)
...
for irep = 1:ntot  % loop over irep
    ...
    %======================== POSTERIOR DRAWS =============================
    % 1.) Draw a|SIGMA,y ~ N(a_post, kron(SIGMA,PHI_post))
    COV = kron(SIGMA_draw,PHI_post);
    a_draw = a_post + chol(COV)'*randn(K*M,1); 
